...
**Scoped session mint:** `POST {AGENT_VAULT_ADDR}/v1/sessions` accepts optional `acting_agent_name` or `acting_agent_id` (mutually exclusive) on a **user** session to mint a vault-scoped token whose proxy identity is that **agent** (for policy grants on the agent). `agent-vault vault run` passes `acting_agent_name` for recognized agent binaries; `agent-vault vault token --acting-agent <name>` does the same without wrapping a process.
